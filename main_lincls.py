...
parser.add_argument('--checkpoint_freq', type=int, default=5, help='checkpoint_freq')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        if args.rank == 0:
            name = f"eval_{args.pretrained.split('/')[-2]}"
            run = wandb.init(
                name=name, 
                project="mixmask",
                config=args,
            ...
            wandb.save('main_lincls.py', policy='now')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_loss, train_acc1, train_acc5 = train(train_loader, model, criterion, optimizer, epoch, args, run)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, folder=args.exp_path)
            if epoch == args.start_epoch:
                sanity_check(model.state_dict(), args.pretrained)
    sys.stdout.flush()
    if run is not None:
        run.log({'val/best_acc1': best_acc1})
        sys.stdout.flush()
        run.finish()
...
def save_checkpoint(state, is_best, folder, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(path, os.path.join(folder, 'model_best.pth.tar'))
